...
class ExchangesController < ApplicationController
  ...
  before_action :authenticate_user!, :except => [:show, :index]
    ...
    if current_user.try(:admin?)
      @open_exchanges = Exchange.select { |e| e.registration_end > DateTime.now }
      @past_exchanges = Exchange.select { |e| e.registration_end < DateTime.now }
    else
      @open_exchanges = Exchange.select { |e| e.registration_end > DateTime.now && e.registration_start <= DateTime.now}
      @past_exchanges = Exchange.select { |e| e.registration_end < DateTime.now && e.registration_start <= DateTime.now}
    end
